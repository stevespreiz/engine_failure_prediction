...
torch.load("./src/features/sparse_ae5.pth")
...
num_units = int(np.max(features_train[:,0]))
...
train_lengths[num_units - 1] = features_train.shape[0] - train_starts[num_units-1]
ind = 0
...
if(True):
    train_windowed = np.zeros((int(np.sum(train_num_windows))*N_tw,features_train.shape[1]))
    row = 0
    for n in range(num_units):
        for j in range(int(train_num_windows[n])):
            ...
            train_windowed[row:(row+N_tw),:] = features_train[start:end,:]
            row += N_tw
    #this is very big file but hopefully can copy to GPU once and then whole thing runs fast
    np.savetxt("./data/processed/windowed_aefeaturestrain1.txt",train_windowed,delimiter=",")
